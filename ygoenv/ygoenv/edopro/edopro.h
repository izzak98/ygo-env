    ...
    public:
        ...
        static decltype(auto) StateSpec(const Config &conf)
        {
            ...
            return MakeDict(
                ...
                "info:num_options"_.Bind(Spec<int>({}, std::make_tuple(0, conf["max_options"_] - 1))),
                "info:to_play"_.Bind(Spec<int>({}, std::make_tuple(0, 1))),
                "info:is_selfplay"_.Bind(Spec<int>({}, std::make_tuple(0, 1))),
                "info:win_reason"_.Bind(Spec<int>({}, std::make_tuple(-1, 1))));
        }
        template <typename Config>
        static decltype(auto) ActionSpec(const Config &conf)
        {
            return MakeDict(
                "action"_.Bind(Spec<int>({}, std::make_tuple(0, conf["max_options"_] - 1))));
        }
    };
